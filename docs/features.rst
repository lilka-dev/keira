...
Запуск DynApp (.so) з SD-картки
-------------------------------

Keira підтримує динамічне завантаження та виконання нативних програм (``.so`` файлів) з SD-карти. Це дозволяє писати програми на C, компілювати їх окремо від Keira і запускати без перепрошивки Лілки.

Файли ``.so`` — це ELF shared objects, скомпільовані для архітектури Xtensa (ESP32-S3). Вбудований ELF-завантажувач розміщує код в PSRAM, виконує релокації та надає доступ до API Keira (дисплей, контролер, зуммер, таймери).

Для компіляції використовується крос-компілятор ``xtensa-esp32s3-elf-gcc`` (входить до складу PlatformIO). Приклад програми та інструкції з компіляції знаходяться в ``examples/dynapp_demo/``.

Детальніше дивіться розділ :ref:`dynapp-native`.

...
Вони знаходяться в репозиторії `lilka-dev/examples <https://github.com/lilka-dev/examples>`_ на GitHub.
...
Якщо в корені карти пам'яті є файл ``autorun.lua`` або ``autorun.js``, Keira запускає його як фоновий сервіс при старті.
Такий скрипт працює у фоні, не має доступу до екрана та кнопок, а його помилки виводяться в серійний порт.

Автозапуск можна вимкнути в меню "Налаштування" → "Сервіси" → "Автозапуск". Вимкнення одразу зупиняє запущені скрипти автозапуску, а увімкнення - запускає їх.

Детальніше - в описі модуля :lua:class:`service`.
